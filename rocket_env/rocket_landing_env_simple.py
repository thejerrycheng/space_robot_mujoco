...
class RocketLandingEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        # Task Constants
        self.TARGET_Z = 0.5
        self.MAX_STEPS = 1000  # Shorter episodes since we don't need persistence
        self.MAX_LATERAL_DIST = 10.0  
        self.MAX_VELOCITY = 20.0      
        self.LANDING_TOLERANCE = 0.5 # Distance from center (0,0) to be considered a hit

        # ----------------------------------------------------------------
        # 3. OBSERVATION & ACTION SPACES
        # ----------------------------------------------------------------
        # [pos(3), vel(3), acc(3), quat(4), ang_vel(3), ang_acc(3), fuel(1)]
    ...
    def _check_termination(self, m):
        ...
        if m["vel_err"] > self.MAX_VELOCITY:
            terminated = True # Unstable/Supersonic
        ...
        if m["z"] < 0.5:
            ...
            # Success Criteria:
            # 1. Within tolerance of target (X/Y)
            # 2. Slow vertical speed (soft landing)
    ...
    def reset(self, seed=None, options=None):
        ...
        # Apply Random Initial State (No Curriculum)
    ...
    # UTILITIES
    # =========================================================================
    def _apply_random_reset(self):
        """
        Resets the rocket to a random state within 'Reasonable Distributions'
        """
        # 1. Position: 5m to 25m altitude, random lateral offset
        alt = np.random.uniform(5.0, 25.0)
        
        # Random position in a circle of radius 15m
        r = np.sqrt(np.random.uniform(0, 15.0**2)) 
        ...
        # 2. Orientation: Random tilt up to 45 degrees
        tilt_deg = np.random.uniform(0, 45.0)
        ...
        # 3. Velocity: Random perturbations
        # Descent velocity: -1.0 to -5.0 m/s
        # Horizontal velocity: -3.0 to 3.0 m/s
        vx = np.random.uniform(-3.0, 3.0)
        vy = np.random.uniform(-3.0, 3.0)
        vz = np.random.uniform(-5.0, -1.0)

        self.data.qvel[self.qvel_adr : self.qvel_adr+3] = [vx, vy, vz]
        self.data.qvel[self.qvel_adr+3 : self.qvel_adr+6] = [0, 0, 0] # No initial spin
