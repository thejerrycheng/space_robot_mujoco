...
# ===== Step Function =====
    ...
    def _flight_control(self, action):
        ...
        ctrl_thrust = action[0] * self.MAX_THRUST
    ...
    def _check_termination(self, state):
        ...
        # Termination condition is triggered when altitude drops below 60.0m
        if state['alt'] < self.TARGET_ALTITUDE + self.LANDING_TOLERANCE_ALT:
            ...
            if is_upright and is_slow and is_close:
                ...
                print("🚀⚠️🟡 Rough Landing!") # Optional logging
            else:
                print("🚀💥❌ Crash Landing!") # Optional logging
                
        # Note: If terminated=True, it overrides truncated=True for standard Gym environments,
        # but both flags are often returned separately in the 'step' function.
        
        # We return both flags: success, terminated, and the new truncated state
    ...
    def _set_fixed_initial_state(self):
        ...
        # --- 1. POSITION --- Applied Domain Randomization --- 
        x = self.INIT_LATERAL_DIST + self.rng.uniform(-60.0, 60.0)  # Add ±20m noise to x position
        y = self.INIT_OFFSET_DIST + self.rng.uniform(-60.0, 60.0)  # Add ±20m noise to y position
        ...
        # --- 3. ORIENTATION ---
        # Yaw: Point the nose roughly toward the target (0,0)
        yaw = np.arctan2(-y, -x) # yaw angle to point at target

        # Pitch: 0 deg is Vertical Up, 90 is Horizontal
        target_pitch_from_up = self.INIT_TILT
        rad_pitch = np.radians(target_pitch_from_up)
        local_z = np.cos(rad_pitch)
        local_fwd = np.sin(rad_pitch)

        # Rolling: zero for simplicity - cannot be controlled directly
