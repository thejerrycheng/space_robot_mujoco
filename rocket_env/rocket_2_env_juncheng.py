...
class RocketLandingEnv(gym.Env):
    ...
    def render(self):
        ...
        # 轨迹少于 2 个点就不用画
        if not hasattr(self, "traj_points") or len(self.traj_points) < 2:
            try:
                self.viewer.sync()
            except:
                pass
        ...
        # 使用 viewer.lock() 修改 user_scn，再 sync()
        with self.viewer.lock():
            ...
            # 模拟“虚线”：隔一个 segment 画一个
            points = self.traj_points
            max_geom = scn.maxgeom  # 防止越界

            # 每两个点算一段线，step=2 就是“虚线效果”
            geom_count = 0
            for i in range(0, len(points) - 1, 2):
                ...
                # 初始化 geom，各种属性在这里设颜色、类型
                mujoco.mjv_initGeom(
                    geom,
                    mujoco.mjtGeom.mjGEOM_LINE,
                    size=np.array([0, 0, 0], dtype=np.float32),   # 对 line 不太重要
                    pos=np.zeros(3, dtype=np.float32),
                    mat=np.eye(3, dtype=np.float32).ravel(),
                    rgba=rgba,
                )

                # 用 mjv_connector 设置“从 p0 到 p1 的线段”
