...
# Import your environment (UPDATED FOR POLAR)
...
# ==============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description="PPO Training for Rocket Landing (Polar)")
    ...
    def trigger_visualization(self):
        ...
        try:
            ...
            while not done:
                ...
                time.sleep(0.01) # Slow down slightly for human viewing
                
            viz_env.close()
            print("🎥 Visualization complete. Resuming training...\n")
        except Exception as e:
            print(f"⚠️ Visualization failed (headless server?): {e}")
...
def main():
    ...
    if args.name:
        # If user provides --name "test", result is "ppo_polar_test_20231024_120000"
        run_name = f"ppo_polar_{args.name}_{timestamp}"
    else:
        # Fallback default
        run_name = f"ppo_polar_{args.reward}_{timestamp}"
        
    log_dir = os.path.join("runs", run_name)
    os.makedirs(log_dir, exist_ok=True)
    
    print(f"🚀 Starting Run: {run_name}")
    print(f"❄️  Environment: Polar Rocket Env")
    ...
    # 4. Callbacks
    ckpt_cb = CheckpointCallback(save_freq=max(50_000 // args.num_envs, 1), save_path=os.path.join(log_dir, "checkpoints"), name_prefix="ppo_polar")
