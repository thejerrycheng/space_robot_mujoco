...
class RocketCallback(BaseCallback):
    """
    Combines Logging (Success Rate) AND Visualization.
    """
    ...
    def _on_step(self) -> bool:
        ...
        if num_dones > 0:
            ...
            for idx, done in enumerate(dones):
                if done and "success" in infos[idx]:
    ...
    def trigger_visualization(self):
        ...
        # We need to recreate the env with the same reward function
        # Luckily, since we injected it via env_kwargs during training, 
        # we can just grab it from the training env's attributes if needed,
        # but here we'll just let the Viz env rely on its internal default 
        # or we could pass it if we stored it. 
        # For simplicity in this callback, we use the env's default behavior 
        # (which we set up to default to flip_and_fuel if None). 
        # If strict consistency is needed, we should pass the function here too.
...
def plot_training_results(log_dir):
    ...
    try:
        ...
        for col in df.columns:
            if 'success' in col: success_col = col; break
        ...
        if success_col:
            ...
            ax2.set_ylabel("Success Rate (%)"); ax2.set_ylim(-5, 105)
...
def main():
    ...
    # Append reward name to run name for clarity
    run_name = args.run_name if args.run_name else f"ppo_{args.reward}_{timestamp}"
    ...
    # env_kwargs allows passing arguments to the Env's __init__
    env = make_vec_env(
        RocketLandingEnv, 
