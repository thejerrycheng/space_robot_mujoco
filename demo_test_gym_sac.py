...
import importlib
import mujoco 
import glob # Added glob for checkpoint finding
...
# NOTE: Removed 'from utils.plotting import generate_interactive_plot, Col' 
# since those functions/classes are defined in this script for self-containment.
...
def generate_interactive_plot(all_histories, save_dir=".", env_name="Rocket Env"):
    ...
    (Function body is unchanged from original)
    """
    try:
...
def main():
    ...
    parser.add_argument("--checkpoint", type=int, default=None, help="Specific timestep checkpoint to load (e.g., 100000)")
    ...
    data_dir = os.path.join(args.run_path, "test_results_sac_demo")
    ...
    if args.checkpoint is not None:
        ckpt_name = f"sac_rocket_{args.checkpoint}_steps.zip"
        model_path = os.path.join(args.run_path, "checkpoints", ckpt_name)
        print(f"{Col.BOLD}Attempting to load specific checkpoint: {ckpt_name}{Col.RESET}")
    ...
    # --- CONTROL CUTOFF PARAMETERS ---
    CUTOFF_ALTITUDE = 55.0 # meters
    CUTOFF_DURATION_SECONDS = 5.0
    
    # Calculate steps based on environment timestep
    CUTOFF_STEPS = int(CUTOFF_DURATION_SECONDS / real_env.DT)
    # --- END CUTOFF PARAMETERS ---

    # --- NEW: Success Tracking Variables ---
    total_episodes = 0
    new_successes = 0
    ...
    for ep in range(args.episodes):
        ...
        cutoff_counter = 0
        # --- END NEW STATE MACHINE VARIABLES ---

        while True:
            step += 1
            
            # Get current position for altitude check
            current_pos = real_env.data.xpos[real_env.rocket_bid].copy()
            altitude = current_pos[2]
            
            # --- START CONTROL CUTOFF LOGIC ---
            if is_cutoff_active:
                if cutoff_counter < CUTOFF_STEPS:
                    # 7. Use a zero action (no thrust, no gimbal) during cutoff
                    action = np.array([0.0, 0.0, 0.0], dtype=np.float32)
                    cutoff_counter += 1
                    status_text = f"{Col.RED}🛑 CUTOFF {cutoff_counter}/{CUTOFF_STEPS}{Col.RESET}"
                else:
                    # Cutoff duration elapsed, return to normal control
                    is_cutoff_active = False
                    status_text = f"{Col.GREEN}✅ CONTROL RE-ESTABLISHED{Col.RESET}"
                    # Re-normalize observation for the SAC model for next step
                    norm_obs = normalize_obs(obs, obs_rms) if obs_rms else obs
                    action, _ = model.predict(norm_obs, deterministic=True)
            
            elif altitude < CUTOFF_ALTITUDE:
                # 6. Altitude reached cutoff threshold
                print(f"\n{Col.RED}🚨 Altitude {altitude:.2f}m < {CUTOFF_ALTITUDE}m. Initiating 5s Control Cutoff!{Col.RESET}")
                is_cutoff_active = True
                cutoff_counter = 1 # Start counter
                # The first action in the cutoff sequence is the zero action
                action = np.array([0.0, 0.0, 0.0], dtype=np.float32)
                status_text = f"{Col.RED}🛑 CUTOFF 1/{CUTOFF_STEPS}{Col.RESET}"

            else:
                # 7. Normal operation: Predict Action from SAC model
                norm_obs = normalize_obs(obs, obs_rms) if obs_rms else obs
                action, _ = model.predict(norm_obs, deterministic=True)
                status_text = f"{Col.CYAN}🚀 SAC CONTROL{Col.RESET}"
            # --- END CONTROL CUTOFF LOGIC ---
            ...
            # Log the action (which is 0.0, 0.0, 0.0 during cutoff)
            thrust_cmd = action[0] 
            g_yaw_cmd = action[1]
            g_pit_cmd = action[2]
            ...
            history['thrust'].append(thrust_cmd)
            history['gimbal'].append([g_yaw_cmd, g_pit_cmd])
            history['mass'].append(mass)
            history['reward'].append(reward)

            # --- NEW: Custom Success Criteria Check ---
            # End of Episode Logging
            final_pos = history['pos'][-1]
            final_vel = history['vel'][-1]
            final_att = history['attitude'][-1] # [Roll, Pitch, Yaw] in degrees
            dist_xy = np.sqrt(final_pos[0]**2 + final_pos[1]**2) # Lateral Distance
            vz = final_vel[2] # Vertical Velocity
            tilt_angle = max(abs(final_att[0]), abs(final_att[1]))
            is_custom_success = (
                (abs(vz) < 50) and               # 1. |Vz| < 50 m/s (using absolute Vz is safer)
                (tilt_angle < 45) and            # 2. Tilt Angle < 45 degrees
                (dist_xy < 100)                  # 3. Distance from center < 100 meters
            )
            total_episodes += 1
            if is_custom_success:
                print(f"{Col.GREEN}🎉 Episode meets CUSTOM SUCCESS criteria!{Col.RESET}")
                new_successes += 1
                custom_result_msg = f"{Col.GREEN}CUSTOM SUCCESS{Col.RESET}"
            else:
                custom_result_msg = f"{Col.YELLOW}CUSTOM FAILURE{Col.RESET}"
            # -----------------------------------------

            if not args.no_render:
                real_env.render()
                state_str = f"Alt:{pos[2]:5.1f}m Dis:{np.sqrt(pos[0]**2 + pos[1]**2):5.1f}m Vz:{vel[2]:5.1f} Tlt:{max(abs(pitch),abs(roll)):4.1f}°"
                ctrl_str  = f"Thr:{thrust_cmd:6.2f} GimY:{g_yaw_cmd:4.2f}"
                log_line = (
                    f"\r{step:04} | {status_text} | {Col.CYAN}{state_str}{Col.RESET} | "
    ...
    generate_interactive_plot(all_histories, data_dir, env_name="SAC Rocket Landing (Demo Cutoff)")
    # --- NEW: Final Success Rate Calculation and Print ---
    success_rate = (new_successes / total_episodes) * 100 if total_episodes > 0 else 0.0

    # Print out in the terminal as requested
    print(f"\n{Col.BOLD}=== FINAL CUSTOM SUCCESS RATE ({Col.CYAN}Vz<50, Tilt<45, Dist<100{Col.RESET}{Col.BOLD}) ==={Col.RESET}")
    print(f"Total Episodes Tested: {total_episodes}")
    print(f"Episodes Meeting Custom Success: {new_successes}")
    print(f"{Col.GREEN}Success Rate: {success_rate:.2f}%{Col.RESET}")
    print(f"=========================================================================")
    # --------------------------------------------------------
    print("\n👋 Testing complete.")
