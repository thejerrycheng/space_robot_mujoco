...
from rocket_env.rocket_landing_env_3 import RocketLandingEnv
...
class VisualizeCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            ...
            # --- SYNC CURRICULUM LEVEL ---
            # We want the visualizer to match the current difficulty of the trainer.
            try:
                # get_attr returns a list of values from all envs in the VecEnv
                levels = self.training_env.get_attr("curriculum_level")
                if levels:
                    current_level = levels[0]
                    self.test_env.curriculum_level = current_level
                    print(f"   Syncing Test Env to Curriculum Level: {current_level}")
            except Exception as e:
                print(f"   Warning: Could not sync curriculum level: {e}")
            # -----------------------------
