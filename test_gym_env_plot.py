        ...
        with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
            ...
            for ep in range(num_episodes):
                ...
                while viewer.is_running() and not (terminated or truncated):
                    ...
                    thrust_cmd = np.random.uniform(-1, 1.0)  # Random thrust between -1 and 1
                    roll_cmd = np.random.uniform(-np.pi/6, np.pi/6)   # Random yaw command (small range in radians)
                    pitch_cmd = np.random.uniform(-np.pi/6, np.pi/6)  # Random pitch command (±30 degrees in radians)
                    action = np.array([thrust_cmd, roll_cmd, pitch_cmd])
                    ...
                    ctrl_str = f"Thr:{thrust_cmd * env.MAX_THRUST:6.0f}N GimY:{np.degrees(roll_cmd):4.1f}°"
                    log_line = (
                    ...
                    history['gimbal'].append([np.degrees(roll_cmd), np.degrees(pitch_cmd)])
