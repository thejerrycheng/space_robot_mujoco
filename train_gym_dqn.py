...
# IMPORT YOUR ENV CLASS (Discrete version for DQN)
...
# ================================================================
def main():
    parser = argparse.ArgumentParser(description="Train DQN Agent for Rocket Landing")
    ...
    parser.add_argument("--max_episode_steps", type=int, default=1000, help="Force episode finish after X steps")
    ...
    # --- System & Logging ---
    parser.add_argument("--num_envs", type=int, default=1) # DQN usually prefers 1 env or small number
    ...
    folder_name = f"gym_dqn_{args.name}_{timestamp}"
    ...
    try:
        ...
        def make_env_instance(render_mode=None):
            ...
            # Note: Using xml_path if your Discrete Env requires it
            env = RocketLandingEnv(render_mode=render_mode) 
            
            # 3a. Apply TimeLimit (Force episode end)
        ...
        # 4. CREATE ENVS
        # DQN works with VecEnv, but traditionally runs well on DummyVecEnv for 1 env
        if args.num_envs > 1:
        ...
        print("------------------------------------------")
        print(f"🚀 TRAINING START | Name: {args.name} | Algo: DQN")
        print("------------------------------------------")
        ...
        model.learn(
            ...
            progress_bar=False # Set False to avoid Mac sys.meta_path error
        )
